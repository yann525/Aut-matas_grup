...
def _analizar_procedure(consulta):
    errores = []
    ...
    if not match:
        return ["Error semantico:\nNo se pudo identificar correctamente el procedimiento."]

    nombre = match.group(1).strip()

    return errores

def _analizar_function(consulta):
    errores = []
    ...
    if not match:
        return ["Error semantico:\nNo se pudo identificar correctamente la función."]

    nombre = match.group(1).strip()

    return errores

def _analizar_trigger(consulta):
    errores = []
    ...
    if not match:
        return ["Error semantico:\nNo se pudo identificar correctamente el trigger."]

    nombre = match.group(1).strip()
    momento = match.group(2).upper()
    evento = match.group(3).upper()
    tabla = match.group(4).strip()

    if not _tabla_existe(tabla):
        errores.append(
            f'Error semantico:\nLa tabla "{tabla}" no existe para el trigger.'
        )
...
def analizar_semantico(consulta):
    ...
    if primera_palabra == "SELECT":
        ...
        if re.search(r"CREATE\s+TABLE", consulta, re.IGNORECASE):
            errores = _analizar_create(consulta)

        elif re.search(r"CREATE\s+PROCEDURE", consulta, re.IGNORECASE):
            errores = _analizar_procedure(consulta)

        elif re.search(r"CREATE\s+FUNCTION", consulta, re.IGNORECASE):
            errores = _analizar_function(consulta)

        elif re.search(r"CREATE\s+TRIGGER", consulta, re.IGNORECASE):
            errores = _analizar_trigger(consulta)

        else:
            errores = [
                'Error semantico:\nTipo de CREATE no reconocido.'
            ]
